...
def save_prediction_to_hierarchy(result: Dict[str, Any], original_text: str, model_type: str, top_k: int, threshold: float) -> bool:
...
# 注意：已删除MOCK预测数据，如需测试模式请使用 /predict/sample 端点获取示例数据
...
async def predict(
    ...
    _: TokenData = Depends(require_permission("predict", "create")),
):
    """实时推理接口（支持小模型API调用和解释生成）"""
    ...
    try:
        ...
        model_type = "hybrid"  # 默认混动模式

        # 处理 caseText：如果它是 JSON 字符串，尝试解析提取 note_text 字段
        case_text = request.caseText
        try:
            # 尝试解析 JSON 字符串
            if case_text.strip().startswith('{') or case_text.strip().startswith('"'):
                parsed = json.loads(case_text)
                if isinstance(parsed, dict) and 'note_text' in parsed:
                    case_text = parsed['note_text']
                elif isinstance(parsed, str):
                    # 如果解析后是字符串，再次尝试解析
                    try:
                        parsed2 = json.loads(parsed)
                        if isinstance(parsed2, dict) and 'note_text' in parsed2:
                            case_text = parsed2['note_text']
                    except:
                        case_text = parsed
        ...
        # 调用模型管理器进行预测
        logger.info(f"开始预测，模型: {model_type}, top_k: {top_k}, threshold: {threshold}")
        ...
        # 转换结果格式以匹配前端期望
        if 'icdPredictions' not in result:
            result['icdPredictions'] = result.get('results', [])

        # 记录返回结果的关键信息
        icd_count = len(result.get('icdPredictions', []))
        has_explanation = bool(result.get('explanation'))
        logger.info(f"预测结果: ICD数量={icd_count}, 是否有解释={has_explanation}")

        if icd_count == 0:
            logger.warning("⚠️ 预测结果为空！可能的原因：1) 小模型API失败 2) AI模型返回空结果 3) 解析失败")
        ...
        logger.info(f"预测完成，返回 {icd_count} 个ICD预测结果")
        return result
